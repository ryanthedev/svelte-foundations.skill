---
description: "Use when troubleshooting errors, debugging build failures, or investigating unexpected behavior."
---
...
| Resource | Path |
|----------|------|
| Vite health check | `${CLAUDE_PLUGIN_ROOT}/skills/_shared/scripts/vite.sh` |
| Browser scripts | `${CLAUDE_PLUGIN_ROOT}/skills/browser/scripts/` |
| Svelte docs | `${CLAUDE_PLUGIN_ROOT}/refs/svelte-docs/` |
| SvelteKit docs | `${CLAUDE_PLUGIN_ROOT}/refs/sveltekit-docs/` |
| Error patterns | `${CLAUDE_PLUGIN_ROOT}/skills/diagnose/references/error-patterns.md` |
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/_shared/scripts/vite.sh status
```
...
Dispatch Agent:
  ...
  prompt: |
    1. Run: ${CLAUDE_PLUGIN_ROOT}/skills/browser/scripts/browser.sh ensure
    2. Run: ${CLAUDE_PLUGIN_ROOT}/skills/browser/scripts/cdp-browser.js screenshot
       Read the output file path to see the image.
    3. Run: ${CLAUDE_PLUGIN_ROOT}/skills/browser/scripts/cdp-browser.js evaluate "JSON.stringify(window.__svelteErrors || [])"
    4. Return:
...
1. Read `${CLAUDE_PLUGIN_ROOT}/skills/diagnose/references/error-patterns.md`
...
Dispatch Agent:
  ...
  prompt: |
    ...
    1. Grep ${CLAUDE_PLUGIN_ROOT}/refs/svelte-docs/ for key error terms
    2. Grep ${CLAUDE_PLUGIN_ROOT}/refs/sveltekit-docs/ for key error terms
